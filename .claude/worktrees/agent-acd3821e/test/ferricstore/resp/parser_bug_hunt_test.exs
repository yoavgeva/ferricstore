defmodule Ferricstore.Resp.ParserBugHuntTest do
  ...
  describe "array with wrong element count" do
    ...
    test "header says 1 element but 3 are present — parses 1, leaves rest" do
      input = "*1\r\n:1\r\n:2\r\n:3\r\n"
      assert {:ok, [[1]], ":2\r\n:3\r\n"} = Parser.parse(input)
    end
  ...
  describe "nested arrays 10 levels deep" do
    ...
    test "10-level nested array where each level has 2 elements" do
      # Each level: *2\r\n<child>\r\n<child>\r\n
      # Leaf level is :1\r\n and :2\r\n
      # This creates 2^10 = 1024 leaf integers, which is fine.
      input = build_nested_binary_tree(10)
      {:ok, [result], ""} = Parser.parse(input)

      # Count all leaf integers
      leaves = count_leaves(result)
      assert leaves == round(:math.pow(2, 10))
    end
  ...
  # At depth 0, returns leaf integers :1\r\n and :2\r\n.
