...
impl Store {
    ...
    /// Returns statistics about this store's on-disk layout.
    ///
    /// The returned tuple contains:
    ///   - `total_disk_bytes`: sum of all `.log` file sizes in the data directory
    ///   - `live_bytes`: approximate bytes occupied by live keydir entries
    ///     (sum of HEADER_SIZE + key_len + value_size for each live entry)
    ///   - `file_count`: number of `.log` files on disk
    ///   - `key_count`: number of live keys in the keydir
    ///   - `dead_bytes`: `total_disk_bytes - live_bytes` (stale + deleted + expired)
    ///   - `fragmentation_ratio`: `dead_bytes / total_disk_bytes` (0.0..1.0), or 0.0
    ///     if `total_disk_bytes` is 0
    ///
    /// These counters drive the merge scheduler's threshold checks.
    #[must_use]
    pub fn shard_stats(&self) -> ShardStats {
        let file_ids = collect_file_ids(&self.data_dir).unwrap_or_default();
        let file_count = file_ids.len() as u64;

        let total_disk_bytes: u64 = file_ids
            .iter()
            .filter_map(|&fid| {
                let p = log_path(&self.data_dir, fid);
                fs::metadata(&p).ok().map(|m| m.len())
            })
            .sum();

        // Estimate live bytes from the keydir: each live entry occupies
        // HEADER_SIZE + key.len() + value_size bytes on disk.
        let mut live_bytes: u64 = 0;
        let mut key_count: u64 = 0;
        for (key, entry) in self.keydir.iter() {
            live_bytes += (HEADER_SIZE + key.len()) as u64 + entry.value_size as u64;
            key_count += 1;
        }

        let dead_bytes = total_disk_bytes.saturating_sub(live_bytes);
        let fragmentation_ratio = if total_disk_bytes > 0 {
            dead_bytes as f64 / total_disk_bytes as f64
        } else {
            0.0
        };
    ...
    /// Returns the on-disk size in bytes of each `.log` file as a sorted vec
    /// of `(file_id, size_bytes)` pairs. Used by the merge scheduler to select
    /// the most fragmented files for incremental merge.
    #[must_use]
    pub fn file_sizes(&self) -> Vec<(u64, u64)> {
        let mut file_ids = collect_file_ids(&self.data_dir).unwrap_or_default();
        file_ids.sort_unstable();
        ...
        file_ids
            ...
            .filter_map(|fid| {
                let p = log_path(&self.data_dir, fid);
                fs::metadata(&p).ok().map(|m| (fid, m.len()))
            })
            .collect()
    }

    /// Returns the path to this store's data directory.
    #[must_use]
    pub fn data_dir(&self) -> &Path {
        &self.data_dir
    }

    /// Run compaction on the specified `file_ids`, merging them into a single
    /// new file. Updates the keydir in-place and deletes old files.
    ///
    /// The `file_ids` must NOT include the active file ID. The caller (Elixir
    /// merge scheduler) is responsible for ensuring no concurrent writes to
    /// the files being compacted.
    ///
    /// Returns the `CompactionOutput` describing what was done.
    ///
    /// # Errors
    ///
    /// Returns a `StoreError` if compaction or file cleanup fails.
    pub fn run_compaction(&mut self, file_ids: &[u64]) -> Result<CompactionResult> {
        ...
        // Compute size of input files before compaction.
        let input_bytes: u64 = file_ids
            .iter()
            .filter_map(|&fid| {
                let p = log_path(&self.data_dir, fid);
                fs::metadata(&p).ok().map(|m| m.len())
        ...
        // Allocate a new file ID strictly greater than any existing file.
        let all_ids = collect_file_ids(&self.data_dir).unwrap_or_default();
        let max_existing = all_ids.iter().copied().max().unwrap_or(0);
        let new_file_id = max_existing + 1;

        let now = now_ms();

        let output = compaction::compact(&self.data_dir, file_ids, &self.keydir, new_file_id, now)
            .map_err(|e| StoreError(e.to_string()))?;

        // Update keydir entries to point to the new compacted file.
        // Re-read the hint file produced by compaction.
        let mut new_kd = KeyDir::new();
        let mut hint_reader = crate::hint::HintReader::open(&output.new_hint_path)
            .map_err(|e| StoreError(e.to_string()))?;
        hint_reader
            .load_into(&mut new_kd)
            .map_err(|e| StoreError(e.to_string()))?;

        // Replace affected keydir entries. Only update entries that still point
        // to one of the compacted file IDs (a concurrent write may have moved
        // a key to the active file — do not overwrite that).
        for (key, new_entry) in new_kd.iter() {
            if let Some(existing) = self.keydir.get(key) {
                if file_ids.contains(&existing.file_id) {
                    self.keydir.put(key.clone(), new_entry.clone());
                }
            }
        }

        // Remove expired keys that were dropped during compaction.
        let expired = self.keydir.expired_keys(now);
        for key in &expired {
            if let Some(entry) = self.keydir.get(key) {
                if file_ids.contains(&entry.file_id) {
                    self.keydir.delete(key);
                }
            }
        }
        ...
        // Delete old files.
        compaction::remove_old_files(&self.data_dir, file_ids)
            .map_err(|e| StoreError(e.to_string()))?;

        // Compute bytes reclaimed.
        let output_bytes = fs::metadata(&output.new_log_path)
            .map(|m| m.len())
            .unwrap_or(0);
        let bytes_reclaimed = input_bytes.saturating_sub(output_bytes);

        Ok(CompactionResult {
            records_written: output.records_written as u64,
            records_dropped: output.records_dropped as u64,
            bytes_reclaimed,
        })
    }

    /// Returns the number of available bytes on the filesystem containing
    /// `data_dir`. Used for pre-merge space checks.
    #[must_use]
    #[allow(unsafe_code)]
    pub fn available_disk_space(&self) -> u64 {
        // Use statvfs on Unix to get available space.
        #[cfg(unix)]
        {
            use std::ffi::CString;
            use std::os::unix::ffi::OsStrExt;

            let path_cstr = CString::new(self.data_dir.as_os_str().as_bytes()).unwrap_or_default();

            // SAFETY: we pass a valid null-terminated path to statvfs and read the
            // result only when the call succeeds (returns 0). The libc::statvfs
            // struct is fully initialized by the kernel on success.
            unsafe {
...
/// Statistics about a store's on-disk layout and fragmentation.
#[derive(Debug, Clone)]
