defmodule Ferricstore.ApplicationTest do
  @moduledoc """
  Tests for the OTP application supervisor tree.

  Verifies that the supervision tree starts correctly, that all expected
  children are present and alive, and that the Ranch TCP listener is
  reachable over a real TCP connection.
  """
  ...
  describe "Ferricstore.Supervisor" do
    ...
    test "has exactly two direct children: ShardSupervisor and Ranch listener" do
      ...
      assert Enum.any?(ids, fn id ->
               id == Ferricstore.Server.Listener or
                 id == {:ranch_embedded_sup, Ferricstore.Server.Listener} or
                 (is_atom(id) and
                    id |> Atom.to_string() |> String.contains?("ranch"))
             end),
             "Expected Ranch listener child in supervisor, got: #{inspect(ids)}"
    end
