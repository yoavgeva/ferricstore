defmodule Ferricstore.Integration.SlidingWindowTest do
  ...
  describe "sliding window delivery" do
    test "fast commands before a slow command get responses delivered first", %{port: port} do
      ...
      # Pipeline: GET (fast, returns nil) -> DEBUG SLEEP 1 (slow) -> PING (fast)
      ...
      # Record when we receive each response
      t0 = System.monotonic_time(:millisecond)

      # Response 0: GET k (fast, should arrive quickly)
      r0 = recv_response(sock)
      t1 = System.monotonic_time(:millisecond)

      # Response 1: DEBUG SLEEP 1 (slow, ~1 second)
      r1 = recv_response(sock)
      t2 = System.monotonic_time(:millisecond)

      # Response 2: PING (fast, but blocked behind DEBUG SLEEP in pipeline order)
      ...
      # Verify timing: GET response should arrive within 100ms (before SLEEP completes)
      get_latency = t1 - t0
      assert get_latency < 500,
             "GET response took #{get_latency}ms, expected < 500ms (should arrive before DEBUG SLEEP)"

      # DEBUG SLEEP should take ~1 second
      sleep_latency = t2 - t0
      assert sleep_latency >= 900,
             "DEBUG SLEEP response took #{sleep_latency}ms, expected >= 900ms"

      # PING response comes after SLEEP (pipeline ordering)
      ping_latency = t3 - t0
      assert ping_latency >= 900,
             "PING response took #{ping_latency}ms, expected >= 900ms (after SLEEP)"
    ...
    test "multiple fast commands before slow command all arrive early", %{port: port} do
      ...
      # Pipeline: GET k1, GET k2, GET k3 (all fast), DEBUG SLEEP 1 (slow)
      pipeline =
        IO.iodata_to_binary([
          Encoder.encode(["GET", k1]),
          Encoder.encode(["GET", k2]),
      ...
      # All 3 GETs should arrive quickly
      r0 = recv_response(sock)
      r1 = recv_response(sock)
      r2 = recv_response(sock)
      t_gets = System.monotonic_time(:millisecond)

      # DEBUG SLEEP arrives later
      r3 = recv_response(sock)
      t_sleep = System.monotonic_time(:millisecond)
      ...
      # All GETs should arrive well before the 1-second sleep
      gets_latency = t_gets - t0
      assert gets_latency < 500,
             "All GET responses took #{gets_latency}ms, expected < 500ms"

      # SLEEP should take ~1 second
      sleep_latency = t_sleep - t0
      assert sleep_latency >= 900,
             "DEBUG SLEEP took #{sleep_latency}ms, expected >= 900ms"
  ...
  describe "MULTI/EXEC in pipeline" do
    test "MULTI/EXEC with queued commands works in pipeline", %{port: port} do
      ...
      assert Enum.at(exec_result, 0) == :ok
