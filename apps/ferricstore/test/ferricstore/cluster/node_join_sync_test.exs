defmodule Ferricstore.Cluster.NodeJoinSyncTest do
  ...
  describe "object storage: new node bootstraps from snapshot" do
    @tag timeout: 120_000
    test "4th node downloads snapshot, WAL bridges the gap, all data synced" do
      # 1. Start 3-node cluster with data
      nodes = ClusterHelper.start_cluster(3)
      on_exit(fn -> ClusterHelper.stop_cluster(nodes) end)
      ...
      keys = write_keys(node_a, "snap_data", 1..100)
      assert_keys_readable(node_b, keys)

      # 2. Configure snapshot_store on node_a using Local adapter
      snapshot_dir = Path.join(System.tmp_dir!(), "ferricstore_snapshots_#{System.unique_integer([:positive])}")
      ...
      :erpc.call(n_a, Application, :put_env, [:ferricstore, :snapshot_store, snapshot_config])

      # 3. Trigger snapshot upload on node_a
      :erpc.call(n_a, Ferricstore.Cluster.SnapshotUploader, :trigger, [])

      # Verify snapshot files exist
      {:ok, files} = File.ls(snapshot_dir)
      assert length(files) > 0, "no snapshot files created"

      # 4. Start 4th node with snapshot_store configured
      #    The node will download from object storage instead of direct copy
      node_d = ClusterHelper.start_node(cluster_nodes: [n_a])
      on_exit(fn -> ClusterHelper.stop_node(node_d) end)

      # Set snapshot_store config on node_d so the join flow uses it
      :erpc.call(node_name(node_d), Application, :put_env, [:ferricstore, :snapshot_store, snapshot_config])
      # Also set on node_a so ClusterManager finds it during join
      :erpc.call(n_a, Application, :put_env, [:ferricstore, :snapshot_store, snapshot_config])

      # 5. Connect — auto-discovery triggers join with snapshot download
      :erpc.call(n_a, Node, :connect, [node_name(node_d)])

      # 6. Write more data after node_d connected
      post_keys = write_keys(node_a, "post_snap", 1..50)
      all_keys = keys ++ post_keys

      # 7. Wait for all keys on node_d
      eventually(fn ->
        missing = Enum.count(all_keys, fn k -> read_key(node_d, k) == nil end)
        assert missing == 0, "#{missing} keys missing on node_d"
      end, "keys not replicated", 60, 500)
      ...
      IO.puts("SUCCESS: #{length(all_keys)} keys synced via object storage snapshot")
    end
  end
