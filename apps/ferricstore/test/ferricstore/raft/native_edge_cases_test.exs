defmodule Ferricstore.Raft.NativeEdgeCasesTest do
  ...
  describe "CAS edge cases through Raft" do
    test "CAS with TTL -- swapped value has correct expiry" do
      ...
      assert 1 = Router.cas(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "original", "swapped", 5_000)
    ...
    test "CAS on expired key -- treats as missing and returns nil" do
      ...
      assert nil == Router.cas(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "expired_val", "new_val", nil)
    ...
    test "CAS concurrent: two CAS on same key -- only one succeeds" do
      ...
      tasks =
        for i <- 1..2 do
          Task.async(fn ->
            Router.cas(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "initial", "winner_#{i}", nil)
          end)
        end
    ...
    test "CAS empty string expected vs nil -- different semantics" do
      ...
      assert 1 = Router.cas(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k_empty, "", "replaced", nil)
      ...
      assert nil == Router.cas(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k_missing, "", "new_val", nil)
  ...
  describe "LOCK edge cases through Raft" do
    test "LOCK on expired key -- acquires lock (treats as free)" do
      ...
      assert :ok = Router.lock(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "new_owner", 30_000)
    ...
    test "LOCK same owner re-acquires -- updates TTL" do
      ...
      assert :ok = Router.lock(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "owner_a", 5_000)
      ...
      assert :ok = Router.lock(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "owner_a", 60_000)
    ...
    test "two different owners -- second gets error" do
      ...
      assert :ok = Router.lock(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "alice", 30_000)

      assert {:error, "DISTLOCK lock is held by another owner"} =
               Router.lock(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "bob", 30_000)
    ...
    test "UNLOCK wrong owner -- error, lock retained" do
      ...
      :ok = Router.lock(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "alice", 30_000)

      assert {:error, "DISTLOCK caller is not the lock owner"} =
               Router.unlock(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "bob")

      # Alice's lock should still be there
      assert "alice" == Router.get(FerricStore.Instance.get(:default), k)

      # Alice can still unlock
      assert 1 = Router.unlock(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "alice")
      assert nil == Router.get(FerricStore.Instance.get(:default), k)
    end
    ...
    test "EXTEND on unlocked key -- error" do
      ...
      assert {:error, "DISTLOCK lock does not exist or has expired"} =
               Router.extend(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "owner_x", 30_000)
    end
  end
  ...
  describe "RATELIMIT.ADD edge cases through Raft" do
    test "window rollover -- old window counts decay" do
      ...
      ["allowed", 5, _rem, _ttl] = Router.ratelimit_add(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, window_ms, max, 5)
      ...
      [status, count, remaining, _ttl] = Router.ratelimit_add(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, window_ms, max, 1)
    ...
    test "multiple rapid calls -- count accurate" do
      ...
      results =
        for i <- 1..10 do
          [_status, count, _rem, _ttl] = Router.ratelimit_add(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, window_ms, max, 1)
          {i, count}
        end
    ...
    test "rate limit with count > 1 -- adds N to counter" do
      ...
      # Add 5 at once
      ["allowed", c1, r1, _ttl] = Router.ratelimit_add(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, window_ms, max, 5)
      assert c1 == 5
      assert r1 == 95

      # Add 3 more
      ["allowed", c2, r2, _ttl] = Router.ratelimit_add(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, window_ms, max, 3)
      assert c2 == 8
      assert r2 == 92

      # Try to add 93 more (would exceed max of 100)
      ["denied", c3, r3, _ttl] = Router.ratelimit_add(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, window_ms, max, 93)
      assert c3 == 8
      assert r3 == 92
    end
    ...
    test "rate limit different keys -- independent windows" do
      ...
      # Exhaust k1
      ["allowed", 5, 0, _] = Router.ratelimit_add(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k1, window_ms, max, 5)
      ["denied", 5, 0, _] = Router.ratelimit_add(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k1, window_ms, max, 1)

      # k2 should be completely independent and still allow
      ["allowed", 1, 4, _] = Router.ratelimit_add(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k2, window_ms, max, 1)
    end
  ...
  describe "concurrent Raft writes -- serialization correctness" do
    ...
    test "5 concurrent CAS on same key -- exactly 1 succeeds" do
      ...
      tasks =
        for i <- 1..5 do
          Task.async(fn ->
            Router.cas(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "starting_value", "cas_winner_#{i}", nil)
          end)
        end
    ...
    test "concurrent LOCK attempts -- exactly 1 acquires" do
      ...
      tasks =
        for i <- 1..5 do
          Task.async(fn ->
            Router.lock(FerricStore.Instance.get(:default), FerricStore.Instance.get(:default), k, "owner_#{i}", 30_000)
          end)
        end
