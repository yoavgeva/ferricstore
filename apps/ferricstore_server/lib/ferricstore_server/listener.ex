defmodule FerricstoreServer.Listener do
  @moduledoc """
  Ranch TCP listener for the FerricStore Redis-compatible server.

  In production the listener is started automatically by
  `FerricstoreServer.Application` via `:ranch.child_spec/5`. It can also be
  started manually in tests using `start/1` / `stop/0`.
